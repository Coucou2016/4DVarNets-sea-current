...
<section id="abstract">
  <h2>1. 摘要</h2>
  <p>海表流场（sea surface currents, SSC；海表面水平流速向量场）的卫星反演长期受限于海表高度（sea surface height, SSH；海面动力高度异常）有效分辨率与地转近似（geostrophy；由 SSH 水平梯度经科氏力平衡得到的地转流）对非地转分量的低估。Fablet 等（2024）提出以可训练观测/先验算子嵌入展开变分同化循环的 <span class="term">4DVarNet</span>（四维变分神经网络求解器）框架，利用海表温度（sea surface temperature, SST）与 SSH 的协同提升流场重建。本工作保持 ConvLSTM 展开求解器不变，仅在变分代价与监督损失中引入：（i）有效表面准地转（effective surface quasi-geostrophy, eSQG-style）速度残差；（ii）SST 平流–扩散残差；（iii）可选的应变相关 UV 异方差不确定性项。</p>
  <p>在受控消融（B2 / M3 / M4）下，合成 OSSE 短训显示 M4 方向性最优；而在 NATL60 <strong>crop96 / 20 epoch</strong> GPU 实验中，实测排序为 <strong>B2 &gt; M3 ≳ M4</strong>（τ_uv：0.848 / 0.811 / 0.801），三者均显著优于地转（τ_uv ≈ −3.74）。M4 曾因 raw NLL 尺度失控导致 SSH 误差恶化，经 σ 归一化修复并重训后 SSH RMSE 恢复至约 0.063。诚实表述为：物理残差可实现且相对地转仍有增益，但在本短裁剪协议上<strong>并非普遍优于纯 SST–SSH 协同</strong>；正式 Table 结论有待全场长训。</p>
</section>
...
<section id="bg">
  ...
  <h3>2.3 研究目标</h3>
  <ol>
    <li>在不改动 4DVarNet 求解器主干的前提下，实现可开关的 SQG / 平流 / 应变不确定性项；</li>
  ...
  <ul>
    ...
    <li><strong>B2 / M3 / M4</strong>：消融编号——B2=SSH+SST；M3=+SQG+平流；M4=+应变 UV 不确定性。</li>
  </ul>
</section>
