...
**日期：** 2026-08-16（P0 更正说明：2026-09-13）  
**证据级别：** 合成短训（方向性）+ NATL60 GPU96 crop96/20ep（**pre_p0_fix / 非正式论文 Table**）  
**基线：** Fablet et al. (2024), JAMES, https://doi.org/10.1029/2023MS003609  
**P0：** 见 `docs/REVIEW_RESPONSE_P0.md`。GPU96 数值在正确性修复前测得，**不得作为正式论文主张**；紧凑求解器为 4DVarNet-inspired，非忠实 Fablet 复现；M4 为应变空间重加权而非不确定性估计。
...
海表流场（SSC）反演受 SSH 分辨率与地转近似限制。本工作在**紧凑的 4DVarNet-inspired**求解器上嵌入 eSQG 风格 SQG 残差、SST 平流残差与可选应变 UV **空间重加权**（M4，非学习不确定性头）。NATL60 **crop96/20ep** 历史实测（**pre_p0_fix**）：**B2 > M3 ≳ M4**（τ_uv 0.848 / 0.811 / 0.801），均优于地转；物理项在本协议上**未超过 B2**。P0 正确性修复后须重训方可主张新分数。
